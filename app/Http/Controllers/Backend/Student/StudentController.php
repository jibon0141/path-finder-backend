...
class StudentController extends Controller
{
    ...
    public function studentReport(Request $request)
    {
        ...
        return view('admin.extends.student.student_report', compact('groups'));
    }

    public function studentReportData(Request $request)
    {
        ...
        if ($request->group_id) {
            $query->where('students.student_group_id', $request->group_id);
        }
        if ($request->age_range) {
            $ages = explode('-', $request->age_range);
            $query->whereBetween('students.age', [$ages[0], $ages[1]]);
        }
